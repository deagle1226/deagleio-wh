...
module.exports.swigFunctions = function(swig) {
            ...
            self.typeInfo[type].controls.forEach(function(control) {
                if (control.controlType === "relation") {
                    relationshipFields.push({
                        name: control.name,
                        isSingle: control.meta.isSingle
                    });
                }
            });
        }
    ...
    var getCombined = function() {
        ...
        names.forEach(function(name) {
            ...
            tempData = _.map(tempData, function(value, key) { << << << < HEAD
                var tmpSlug = "";
                ...
                return value >>> >>> > 3c5b8f9a286b5e958540476be8d6aafb98c04428
            });
    ...
    var nextItem = function(item, sort_name, reverse_sort) {
        ...
        items.some(function(itm) { << << << < HEAD
                if (previousItem && previousItem._id == item._id) { === === =
                    if (previousItem && previousItem.name == item.name) { >>> >>> > 3c5b8f9a286b5e958540476be8d6aafb98c04428
                        nextItem = itm;
                        return true;
                    }
        ...
        var prevItem = function(item, sort_name, reverse_sort) {
            ...
            items.some(function(itm) { << << << < HEAD
                    if (itm._id == item._id) { === === =
                        if (itm.name == item.name) { >>> >>> > 3c5b8f9a286b5e958540476be8d6aafb98c04428
