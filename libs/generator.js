...
module.exports.generator = function(config, logger, fileParser) {
    ...
    var writeTemplate = function(inFile, outFile, params) {
        ...
        swigFunctions.setParams({
            CURRENT_URL: outputUrl
        });
    ...
    var resetGenerator = function(callback) {
        ...
        repoRequest
            ...
            .on('close', function() {
                ...
                entries.forEach(function(entry) {
                    if (entry.entryName.indexOf('pages/') === 0 || entry.entryName.indexOf('templates/') === 0 || entry.entryName.indexOf('static/') === 0) {
                        ...
                        if (baseName === 'list') {
                            ...
                            for (var key in publishedItems) {
                                ...
                                if (fs.existsSync(overrideFile)) {
                                    writeTemplate(overrideFile, newPath, {
                                        item: val
                                    });
                                } else {
                                    writeTemplate(file, newPath, {
    ...
    this.copyStatic = function(callback) {
        ...
        if (fs.existsSync('static')) {
            ...
            wrench.copyDirSyncRecursive('static', '.build/static', {
                forceDelete: true
            });
        }
        callback();
    ...
    this.makeScaffolding = function(name, done, force) {
        ...
        var renderWidget = function(controlType, fieldName, controlInfo) {
            var widgetString = _.template(fs.readFileSync('./libs/widgets/' + controlType + '.html'), {
        ...
        getData(function(data, typeInfo) {
            ...
            if (typeInfo[name].oneOff) {
                ...
                var oneOffFile = _.template(oneOffTemplate, {
                ...
                var template = _.template(individualTemplate, {
                    widgetFiles: widgetFiles,
                    typeName: name,
                    typeInfo: typeInfo[name] || {},
                    controls: controlsObj
                ...
                var lTemplate = _.template(listTemplate, {
                    typeName: name
                });
    ...
    this.reloadFiles = function(done) {
        request({
            url: 'http://localhost:' + liveReloadPort + '/changed?files=true',
            timeout: 10
            ...
            sock.on('message', function(message) {
                if (message.indexOf('scaffolding:') === 0) {
                    ...
                    resetGenerator(function(error) {
                        if (error) {
                            sock.send('done:' + JSON.stringify({
                                err: 'Error while resetting files'
                            }));
                        } else {
                            sock.send('done');
                        }
